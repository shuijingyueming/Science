...
$(function () {
    ...
    jQuery.validator.addMethod("isMobile1", function (value, element) {
        var length = value.length;
        return this.optional(element) || (length == 11 && /^((1[1-9][0-9]{1})+\d{8})$/.test(value));
    }, "手机号码格式不正确。");
    ...
    jQuery.validator.addMethod("stringMaxLength", function (value, element, params) {
        ...
        return length > params[1] || length < params[0] ? false : true;
    }, "一个汉字为两个字符");
    //验证是不是纯数字
    jQuery.validator.addMethod("isCNumber", function (value, element, params) {
    ...
    jQuery.validator.addMethod("isIntGtZero1", function (value, element) {
        console.log(parseInt(value) != parseFloat(value))
        return this.optional(element)||parseInt(value) == parseFloat(value)&& parseInt(value) > 0;
    }, "整数必须大于0");
